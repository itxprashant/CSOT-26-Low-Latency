...
//  WEEK 5 NOTE:
//  ------------
//  This header is carried forward UNCHANGED from Week 1 (the spec is frozen for
//  the whole track). In Week 5 your strategy is still the *consumer* end of a
//  lock-free pipeline — but the producer is now a network ingest thread that
//  reads csot::WireTick records off a UDP loopback socket via an epoll loop and
//  hands the decoded csot::Tick records across an SPSC ring buffer to a thread
//  that drives exactly this Strategy. See ../GATEWAY_SPEC.md for how the wire,
//  the ring, and the strategy fit together in one gateway.cpp.
// ============================================================================
